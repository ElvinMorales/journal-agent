...
| Tools | `scripts/`, `mcp_server/`, `docs/mcp-local-server.md`, `docs/future-mcp-vps-controller-contract.md` | Validation, redaction, private-vault initialization, and the narrow local MCP tool surface | Public tooling and specifications; configured vaults and private outputs remain outside Git | Initializer and minimal local MCP tools covered; exact-wording apply remains disabled |
| Knowledge/resources | `references/`, `docs/`, `docs/guided-intake.md`, `docs/obsidian-private-runtime-guide.md`, `docs/private-vault-runtime-package.md`, `docs/mcp-local-server.md`, `docs/v0.2-usable-product-handoff.md` | Supporting references, guided-intake guidance, private-runtime initialization, local MCP operation, and public-reader handoff | Public-safe guidance only; intake answers and private runtime content stay outside Git | Covered for intake, private setup, local MCP use, and v0.2 handoff |
...
| Memory | `MEMORY.md.example`, `scripts/init-private-vault.py`, `mcp_server/`, `docs/mcp-local-server.md`, `schemas/memory-update-proposal.schema.json`, `examples/memory-state-proposals/`, `docs/memory-state-proposal-review.md`, `private/memory/` | Durable approved context, allowlisted local reads, separate inert pending proposals, and a disabled exact-wording apply boundary | Runtime code public; generated starters, real Memory, and proposals private and outside Git | Read and pending-proposal boundaries covered; apply planned for issue #31 |
| State | `STATE.md.example`, `scripts/init-private-vault.py`, `mcp_server/`, `docs/mcp-local-server.md`, `schemas/state-update-proposal.schema.json`, `examples/memory-state-proposals/`, `docs/memory-state-proposal-review.md`, `private/state/` | Temporary context, allowlisted local reads, separate trigger-bearing pending proposals, and a disabled exact-wording apply boundary | Runtime code public; generated starters, live State, and proposals private and outside Git | Read and pending-proposal boundaries covered; apply planned for issue #31 |
| Planning/orchestration | `prompts/guided-intake.md`, `docs/guided-intake.md`, `docs/journal-mirror-workflow.md`, `docs/private-vault-runtime-package.md`, `mcp_server/`, `docs/mcp-local-server.md`, `docs/memory-state-proposal-review.md`, `docs/roadmap-v0.3.0.md` | Guided onboarding, selected-context reflection, private setup, proposal lifecycle/status, disabled apply gate, and sprint sequencing | Public instructions and code only; generated vaults, intake answers, sessions, and proposals remain private | Intake, sessions, initialization, and minimal local MCP flow covered |
| Guardrails/governance | `GUARDRAILS.md`, `PRIVACY.md`, `SECURITY.md`, `CONTRIBUTING.md`, `scripts/init-private-vault.py`, `mcp_server/vault_runtime.py`, `tests/test_mcp_server_boundaries.py`, `docs/mcp-local-server.md`, `evals/`, `docs/decisions/`, `docs/future-mcp-vps-controller-contract.md` | Safety, privacy, path refusal, scope allowlists, denied scans and arbitrary access, no silent writes, exact-wording approval boundaries, and synthetic tests | Public rules, code, contracts, and synthetic tests; no sensitive runtime artifacts | Local MCP denials are implemented and tested; exact-wording apply remains future work |
...
| Evaluation/observability | `tests/test_init_private_vault.py`, `tests/test_mcp_server_boundaries.py`, `EVALS.md`, `evals/`, `examples/intake/`, `examples/journal-mirror-walkthroughs/`, `examples/memory-state-proposals/` | Automated initializer and local MCP boundary tests, metadata-only audit behavior, manual regression, safety review, and release verification | Public-safe tests and synthetic cases only; private traces, content logs, and clinical validation excluded | Initializer and minimal MCP boundary tests covered; expanded eval work remains planned |
| Runtime/deployment | `mobile/`, `.gitignore`, `.github/workflows/`, `scripts/init-private-vault.py`, `mcp_server/`, `requirements.txt`, `tests/test_mcp_server_boundaries.py`, `docs/mcp-local-server.md`, `docs/private-vault-runtime-package.md`, `docs/future-mcp-vps-controller-contract.md`, `docs/roadmap-v0.3.0.md` | Private-vault initialization and the local stdio MCP runtime edge within the control/data-plane architecture | Runtime code is public; configured paths, generated vaults, and private data stay outside the repo | Minimal local MCP server covered; connector, hosting, tunnel, viewer, and apply remain future work |
| Learning/iteration | `CHANGELOG.md`, `BACKLOG.md`, `EVALS.md`, `tests/test_mcp_server_boundaries.py`, `docs/roadmap-v0.3.0.md`, `docs/guided-intake.md`, `docs/decisions/0003-journal-mirror-mcp-runtime.md`, `docs/memory-state-proposal-review.md`, `docs/future-mcp-vps-controller-contract.md` | Change history, executable boundary lessons, review criteria, roadmap decisions, and future sprint work | Public project tracking and synthetic evidence only | `v0.2.0` released; issue #30 local server addressed; issue #31 apply is next |
...
- The minimal local MCP runtime and boundary tests are present; exact-wording apply, connector setup, expanded runtime evals, and the local viewer remain separate follow-up issues.
...
Guided intake maps across Prompts/interfaces, Knowledge/resources, Memory, State, Planning/orchestration, Guardrails/governance, Outputs/schemas, Evaluation/observability, and Learning/iteration. The private-vault package and minimal local MCP server map across Tools, Knowledge/resources, Memory, State, Planning/orchestration, Guardrails/governance, Evaluation/observability, Runtime/deployment, and Learning/iteration. These mappings do not create new top-level categories; the taxonomy remains exactly 14 buckets.
